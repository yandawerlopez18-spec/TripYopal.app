    ...
    <div className="rounded-2xl border border-forest-700 bg-forest-950 p-5">
            ...
            {editingId === event.id ? (
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                <div className="flex gap-2 sm:col-span-2">
                  <button type="button" onClick={() => saveEdit(event.id)} className="btn-gradient rounded-full px-3 py-1.5 text-xs font-semibold text-forest-950">
                    Guardar
                  </button>
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-slate-400">{formatEventDate(event.date)} · {event.place}</p>
                  </div>
                </div>
                <div className="flex shrink-0 gap-2">
                  <button type="button" onClick={() => startEdit(event)} className="btn-brand-font btn-gradient rounded-full px-3 py-1.5 text-xs font-semibold text-forest-950 transition">
                    Editar
                  </button>
      ...
      {showAddForm ? (
        <form onSubmit={handleAddSubmit} className="mt-5 grid gap-3 border-t border-forest-700 pt-5 sm:grid-cols-2">
          ...
          <input className={inputClass} type="date" value={addForm.date} onChange={(e) => setAddForm({ ...addForm, date: e.target.value })} />
