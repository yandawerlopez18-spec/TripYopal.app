...
import RapidaOwnerEditor from "./RapidaOwnerEditor";
import {
  ArrowUpRightIcon,
  CalendarIcon,
  CheckIcon,
...
const COMPACT_FEATURE_KEYS = ["comidaRapidaFeature", "bebidasFeature", "ambienteFeature", "terrazaAireLibreFeature", "wifi", "petFriendly", "preciosAccesibles"];
const SIDEBAR_SERVICE_KEYS = ["wifiGratuito", "pantallasGigantes", "musica", "terrazaAireLibreSidebar", "zonaFumadores", "parqueaderoCercano", "reservaEventos"];

const TRUST_BADGES = [
  { icon: CheckIcon, title: "Información verificada", subtitle: "Datos 100% reales" },
  { icon: HeartIcon, title: "Excelente ambiente", subtitle: "Vive una gran experiencia" },
  { icon: HeadsetIcon, title: "Atención 24/7", subtitle: "Estamos para ayudarte" },
  { icon: ShieldIcon, title: "Pagos seguros", subtitle: "Tu seguridad es primero" },
  { icon: ArrowUpRightIcon, title: "Apoyamos lo local", subtitle: "Hecho en Yopal" },
];
...
function priceRangeInfo(priceRange?: string): { symbol: string; label: string } | null {
  ...
  if (priceRange === "Alto") return { symbol: "$$$", label: "Alto" };
  return null;
}
...
export default function RapidaDetailView({
  ...
  const hasImportantInfo = Boolean(prestador.schedule || prestador.cuisineType || prestador.bestTimeToVisit || prestador.priceRange || prestador.keyServices || prestador.idealFor);
  ...
  return (
    <main className="min-h-screen bg-forest-950 px-4 py-4 text-slate-100 lg:px-8">
      <div className="mx-auto max-w-8xl">
        ...
        {canEdit && editMode ? <RapidaOwnerEditor prestadorId={prestador.id} categoryKey={categoryKey} onSaved={() => setDataVersion((v) => v + 1)} /> : null}
        ...
        <div className="mt-4 grid gap-6 lg:grid-cols-[1fr_360px]">
          <div>
            ...
            <div className="mt-6">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-brand-400">Comidas rápidas</p>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-300">
                ...
                <span className="flex items-center gap-1.5">
                  <HeartIcon className="h-4 w-4 text-brand-400" /> Excelente ambiente
                </span>
                <span className="flex items-center gap-1.5">
                  <TagIcon className="h-4 w-4 text-brand-400" /> Precios accesibles
                </span>
                <span className="flex items-center gap-1.5">
                  <UsersIcon className="h-4 w-4 text-brand-400" /> Ideal para grupos
                </span>
              </div>
            ...
            {items.length > 0 || highlights.length > 0 ? (
              <div className="mt-6 grid gap-6 lg:grid-cols-2">
                ...
                {highlights.length > 0 ? (
                  <div className="rounded-2xl border border-forest-700 bg-forest-900 p-6">
                    <h2 className="text-lg font-bold text-slate-100">Lo que más disfrutan</h2>
                    <ul className="mt-4 space-y-2.5 text-sm text-slate-300">
                      {highlights.map((highlight) => (
                        <li key={highlight.id} className="flex items-center gap-2">
                          <span className="text-base">{highlight.icon}</span> {highlight.title}
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-3">
                  {venueEvents.map((event) => (
                    <div key={event.id} className="overflow-hidden rounded-xl border border-forest-700 bg-forest-950">
                      {event.imageUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={event.imageUrl} alt={event.title} className="h-24 w-full object-cover" />
                      ) : (
                      ...
                      <div className="p-3">
                        ...
                        <p className="mt-0.5 text-xs capitalize text-brand-400">{formatEventDate(event.date)}</p>
                        {event.time ? <p className="text-xs text-slate-400">{event.time}</p> : null}
                        {event.description ? <p className="mt-1 line-clamp-2 text-xs text-slate-400">{event.description}</p> : null}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-x-8 gap-y-3 text-sm text-slate-200">
                  {prestador.address ? (
                    <span className="flex items-center gap-2">
                      <CompassIcon className="h-4 w-4 text-brand-400" /> {prestador.address}
                    </span>
                  ) : null}
                  {prestador.phone ? (
                  ...
                  {prestador.facebook ? (
                    <a
                      ...
                      className="flex items-center gap-2 hover:text-brand-400"
                    >
                      <FacebookIcon className="h-4 w-4 text-brand-400" /> {prestador.name}
                    </a>
                  ) : null}
                  {prestador.email ? (
          ...
          <div className="space-y-6 lg:sticky lg:top-6 lg:self-start">
            {prestador.whatsapp ? (
              <div id="reserva" className="relative scroll-mt-20 overflow-hidden rounded-2xl border border-forest-700">
                ...
                <div className="relative flex min-h-[220px] flex-col justify-end p-6">
                  <p className="text-lg font-bold text-white">¿Planeando tu noche?</p>
                  <p className="mt-2 text-sm text-slate-200">Reserva tu mesa para eventos especiales o cumpleaños. ¡Te esperamos!</p>
                  <a
                    href={`https://wa.me/${prestador.whatsapp}?text=${encodeURIComponent(`Hola, quiero reservar en ${prestador.name}.`)}`}
                    target="_blank"
                    rel="noreferrer"
                    className="btn-brand-font btn-gradient mt-4 flex items-center justify-center gap-2 rounded-full py-2.5 text-sm font-semibold text-forest-950 transition"
                  >
            ...
            <div className="rounded-2xl border border-forest-700 bg-forest-900 p-6 text-center">
              ...
              <p className="mt-1 text-xs text-slate-400">¿Necesitas recomendaciones de comidas rápidas en Yopal? Estoy aquí para ayudarte.</p>
              <button
                type="button"
                onClick={openChat}
                  ...
                  {prestador.nearbyPlaces!.slice(0, 4).map((place) => (
                    <div key={place.id} className="flex items-center justify-between text-slate-300">
                      <span className="flex items-center gap-1.5 truncate">
                        <CompassIcon className="h-3.5 w-3.5 shrink-0 text-brand-400" /> {place.name}
                      </span>
            ...
            {hasImportantInfo ? (
              <div className="rounded-2xl border border-forest-700 bg-forest-900 p-6">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
                  ...
                  {prestador.cuisineType ? (
                    <div className="flex items-start gap-2">
                      <TagIcon className="mt-0.5 h-4 w-4 shrink-0 text-brand-400" />
                      <span><span className="block text-slate-500">Tipo de cocina</span><span className="text-slate-200">{prestador.cuisineType}</span></span>
                    </div>
                  ) : null}
