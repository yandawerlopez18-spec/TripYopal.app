...
import ParrillaOwnerEditor from "./ParrillaOwnerEditor";
import {
  ArrowUpRightIcon,
  CalendarIcon,
  CheckIcon,
...
const GUEST_ASPECTS: { key: keyof ReviewAspects; label: string }[] = [
  { key: "cleanliness", label: "Sabor" },
  { key: "service", label: "Atención" },
  { key: "comfort", label: "Ambiente" },
  { key: "price", label: "Precio" },
  { key: "facilities", label: "Porciones" },
];

const COMPACT_FEATURE_KEYS = ["carnesAlCarbon", "costillaCerdo", "chorizosFeature", "polloFeature", "acompanamientos", "bebidasParrilla", "ambienteLlanero", "parqueaderoAmplio"];
const SIDEBAR_SERVICE_KEYS = ["parqueaderoGratuito", "zonaInfantil", "wifiGratuito", "banosLimpios", "mesasAireLibre", "eventosCelebraciones", "pagoTarjetaEfectivo"];

const TRUST_BADGES = [
  { icon: CheckIcon, title: "Información verificada", subtitle: "Datos 100% reales" },
  { icon: StarIcon, title: "Restaurante recomendado", subtitle: "Por TripYopal" },
  { icon: HeadsetIcon, title: "Atención 24/7", subtitle: "Estamos para ayudarte" },
  { icon: HeartIcon, title: "Sabor que enamora", subtitle: "Carnes al carbón" },
...
export default function ParrillaDetailView({
  ...
  return (
    <main className="min-h-screen bg-forest-950 px-4 py-4 text-slate-100 lg:px-8">
      <div className="mx-auto max-w-8xl">
        ...
        {canEdit && editMode ? <ParrillaOwnerEditor prestadorId={prestador.id} categoryKey={categoryKey} onSaved={() => setDataVersion((v) => v + 1)} /> : null}
        ...
        <div className="mt-4 grid gap-6 lg:grid-cols-[1fr_360px]">
          <div>
            ...
            <div className="mt-6">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-brand-400">Asadero</p>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-300">
                ...
                <span className="flex items-center gap-1.5">
                  <HeartIcon className="h-4 w-4 text-brand-400" /> Excelente sabor
                </span>
                <span className="flex items-center gap-1.5">
                  <TagIcon className="h-4 w-4 text-brand-400" /> Carnes de calidad
                </span>
                <span className="flex items-center gap-1.5">
                  <TagIcon className="h-4 w-4 text-brand-400" /> Precios justos
                </span>
                <span className="flex items-center gap-1.5">
                  <UsersIcon className="h-4 w-4 text-brand-400" /> Ideal para familias
                </span>
              </div>
            ...
            {venueEvents.length > 0 ? (
              <div className="mt-6 rounded-2xl border border-forest-700 bg-forest-900 p-6">
                <div className="flex items-center justify-between">
                  <h2 className="text-lg font-bold text-slate-100">Eventos y promociones</h2>
          ...
          <div className="space-y-6 lg:sticky lg:top-6 lg:self-start">
            {prestador.whatsapp ? (
              <div id="reserva" className="relative scroll-mt-20 overflow-hidden rounded-2xl border border-forest-700">
                ...
                <div className="relative flex min-h-[220px] flex-col justify-end p-6">
                  <p className="text-lg font-bold text-white">¿Vas a celebrar algo especial?</p>
                  <p className="mt-2 text-sm text-slate-200">Reserva tu mesa para cumpleaños, reuniones o eventos especiales. ¡Te esperamos!</p>
                  <a
            ...
            <div className="rounded-2xl border border-forest-700 bg-forest-900 p-6 text-center">
              ...
              <p className="mt-1 text-xs text-slate-400">¿Buscas el mejor asadero en Yopal? Estoy aquí para recomendarte los mejores lugares.</p>
              <button
                type="button"
                onClick={openChat}
