...
export default function ParqueOwnerEditor({ prestadorId, categoryKey, onSaved }: { prestadorId: string; categoryKey: string; onSaved: () => void }) {
  ...
  return (
    <div className="mt-6 rounded-3xl border border-brand-500/40 bg-forest-900 p-6">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-bold text-slate-100">Panel del administrador del parque</h2>
      </div>
      ...
      <div className={sectionClass}>
        ...
        <form onSubmit={handleGeneralSubmit} className="mt-4 grid gap-3 sm:grid-cols-2">
          <input className={inputClass} placeholder="Nombre del parque" value={generalForm.name} onChange={(e) => setGeneralForm({ ...generalForm, name: e.target.value })} />
          ...
          <input className={inputClass} placeholder="Horario (ej. Abierto todos los días 5:00 a.m. - 10:00 p.m.)" value={generalForm.schedule} onChange={(e) => setGeneralForm({ ...generalForm, schedule: e.target.value })} />
      ...
      <div className={sectionClass}>
        <h3 className="font-semibold text-slate-100">Próximos eventos en el parque</h3>
        <p className="mt-1 text-sm text-slate-400">Agrega actividades y eventos que se realizarán próximamente en este parque (yoga, conciertos, ferias, etc.).</p>
        ...
        <form onSubmit={handleEventSubmit} className="mt-4 grid gap-3 border-t border-forest-700 pt-4 sm:grid-cols-2">
          ...
          <input className={inputClass} placeholder="Hora (ej. 7:00 a.m.)" value={eventForm.time} onChange={(e) => setEventForm({ ...eventForm, time: e.target.value })} />
          <ImageUploadField value={eventForm.imageUrl} onChange={(value) => setEventForm({ ...eventForm, imageUrl: value })} />
          <textarea
            className={`${inputClass} sm:col-span-2`}
            placeholder="Descripción breve (ej. Actividad gratuita)"
