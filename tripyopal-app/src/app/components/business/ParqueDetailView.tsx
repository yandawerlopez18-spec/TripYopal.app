...
import ParqueOwnerEditor from "./ParqueOwnerEditor";
import {
  ArrowUpRightIcon,
  CheckIcon,
  ChevronLeftIcon,
...
const GUEST_ASPECTS: { key: keyof ReviewAspects; label: string }[] = [
  ...
  { key: "location", label: "Accesibilidad" },
...
export default function ParqueDetailView({
  ...
  const hasParkInfo = Boolean(prestador.parkArea || prestador.foundingYear || prestador.parkType || prestador.managedBy);
  ...
  const otherParks = listPrestadoresByCategory(categoryKey)
      ...
      <div className="mx-auto max-w-8xl">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {canEdit ? (
            ...
            >
              {editMode ? "Salir del modo edición" : "Editar información del parque"}
            </button>
          ) : null}
        </div>
        ...
        {canEdit && editMode ? <ParqueOwnerEditor prestadorId={prestador.id} categoryKey={categoryKey} onSaved={() => setDataVersion((v) => v + 1)} /> : null}
        ...
        <div className="mt-4 grid gap-6 lg:grid-cols-[1fr_360px]">
          <div>
            ...
            <div className="mt-6">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-brand-400">Parque</p>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-300">
                ...
                <span className="flex items-center gap-1.5">
                  <EventPinIcon className="h-4 w-4 text-brand-400" /> Muy popular en Yopal
                </span>
                <span className="flex items-center gap-1.5">
                ...
                {prestador.entryFee ? (
                  <span className="flex items-center gap-1.5">
                    <TagIcon className="h-4 w-4 text-brand-400" /> {prestador.entryFee}
                  </span>
                ) : null}
              </div>
                ...
                <div className="mt-4 grid grid-cols-3 gap-4">
                  {visibleActivities.map((activity) => (
                    <div key={activity.id} className="overflow-hidden rounded-xl border border-forest-700 bg-forest-950">
                      {activity.imageUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={activity.imageUrl} alt={activity.name} className="h-32 w-full object-cover" />
                      ) : (
                      ...
                      <div className="p-2.5">
                        ...
                        {activity.description ? <p className="mt-0.5 line-clamp-2 text-[11px] leading-tight text-slate-400">{activity.description}</p> : null}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* Información del parque + Lo que nos hace únicos */}
            {hasParkInfo || highlights.length > 0 ? (
              <div className="mt-6 grid gap-6 lg:grid-cols-2">
                {hasParkInfo ? (
                  <div className="rounded-2xl border border-forest-700 bg-forest-900 p-6">
                    <h2 className="text-lg font-bold text-slate-100">Información del parque</h2>
                    <div className="mt-4 divide-y divide-forest-700 text-sm">
                      <div className="flex items-center justify-between py-2.5">
                        <span className="text-slate-400">Nombre oficial</span>
                        <span className="text-right text-slate-100">{prestador.name}</span>
                      </div>
                      {prestador.parkArea ? (
                        <div className="flex items-center justify-between py-2.5">
                          <span className="text-slate-400">Área aproximada</span>
                          <span className="text-right text-slate-100">{prestador.parkArea}</span>
                        </div>
                      ) : null}
                      {prestador.foundingYear ? (
                        <div className="flex items-center justify-between py-2.5">
                          <span className="text-slate-400">Año de fundación</span>
                          <span className="text-right text-slate-100">{prestador.foundingYear}</span>
                        </div>
                      ) : null}
                      {prestador.parkType ? (
                        <div className="flex items-center justify-between py-2.5">
                          <span className="text-slate-400">Tipo de parque</span>
                          <span className="text-right text-slate-100">{prestador.parkType}</span>
                        </div>
                      ) : null}
                      {prestador.managedBy ? (
                        <div className="flex items-center justify-between py-2.5">
                          <span className="text-slate-400">Administrado por</span>
                          <span className="text-right text-slate-100">{prestador.managedBy}</span>
                        </div>
                      ) : null}
                    </div>
                  </div>
            ...
            {upcomingVenueEvents.length > 0 ? (
              <div className="mt-6 rounded-2xl border border-forest-700 bg-forest-900 p-6">
                <div className="flex items-center justify-between">
                  <h2 className="text-lg font-bold text-slate-100">Próximos eventos en el parque</h2>
            ...
            <div id="opiniones" className="mt-6 scroll-mt-20 rounded-2xl border border-forest-700 bg-forest-900 p-6">
              ...
              {averageRating !== null ? (
                ...
                <p className="mt-4 text-sm text-slate-500">Aún no hay opiniones para este parque.</p>
              )}
          ...
          <div className="space-y-6 lg:sticky lg:top-6 lg:self-start">
            {prestador.schedule || prestador.entryFee || prestador.bestTimeToVisit || prestador.averageClimate || prestador.visitRecommendations || prestador.safetyNote || prestador.idealFor ? (
              <div className="rounded-2xl border border-forest-700 bg-forest-900 p-6">
                ...
                <div className="mt-4 grid grid-cols-1 gap-4 text-sm">
                  ...
                  {prestador.entryFee ? (
                    <div className="flex items-start gap-3">
                      ...
                      <span><span className="block text-slate-500">Entrada</span><span className="text-slate-200">{prestador.entryFee}</span></span>
                    </div>
                  ) : null}
                  ...
                  {prestador.safetyNote ? (
                    <div className="flex items-start gap-3">
                      <ShieldIcon className="mt-0.5 h-5 w-5 shrink-0 text-brand-400" />
                      <span><span className="block text-slate-500">Seguridad</span><span className="text-slate-200">{prestador.safetyNote}</span></span>
                    </div>
                  ) : null}
                  {prestador.idealFor ? (
                    <div className="flex items-start gap-3">
            ...
            <div className="rounded-2xl border border-forest-700 bg-forest-900 p-6 text-center">
              ...
              <p className="mt-1 text-xs text-slate-400">¿Necesitas recomendaciones de parques en Yopal? Estoy aquí para ayudarte.</p>
              <button
                type="button"
                onClick={openChat}
