...
export default function ParrillaOwnerEditor({ prestadorId, categoryKey, onSaved }: { prestadorId: string; categoryKey: string; onSaved: () => void }) {
    ...
    <div className="mt-6 rounded-3xl border border-brand-500/40 bg-forest-900 p-6">
      ...
      <div className={sectionClass}>
        ...
        <form onSubmit={handleGeneralSubmit} className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <input className={inputClass} placeholder="Horario de atención (ej. 8:00 a.m. - 4:00 p.m.)" value={generalForm.schedule} onChange={(e) => setGeneralForm({ ...generalForm, schedule: e.target.value })} />
      ...
      <div className={sectionClass}>
        <h3 className="font-semibold text-slate-100">Eventos y promociones</h3>
        <p className="mt-1 text-sm text-slate-400">Agrega tus eventos y promociones recurrentes (música en vivo, día del asado, cumpleaños, etc.). Usa el campo &ldquo;Fecha&rdquo; para describir la recurrencia (ej. &ldquo;Todos los sábados&rdquo;).</p>
        ...
        <form onSubmit={handleEventSubmit} className="mt-4 grid gap-3 border-t border-forest-700 pt-4 sm:grid-cols-2">
          <input className={inputClass} placeholder="Título (ej. Música en vivo)" value={eventForm.title} onChange={(e) => setEventForm({ ...eventForm, title: e.target.value })} />
          <input className={inputClass} placeholder="Fecha o recurrencia (ej. Todos los sábados)" value={eventForm.date} onChange={(e) => setEventForm({ ...eventForm, date: e.target.value })} />
          <input className={inputClass} placeholder="Hora o detalle (ej. Desde 7:00 p.m.)" value={eventForm.time} onChange={(e) => setEventForm({ ...eventForm, time: e.target.value })} />
          ...
          />
          <button className="btn-gradient rounded-full px-4 py-3 text-sm font-semibold text-forest-950 transition sm:col-span-2">Agregar evento o promoción</button>
        </form>
      </div>
