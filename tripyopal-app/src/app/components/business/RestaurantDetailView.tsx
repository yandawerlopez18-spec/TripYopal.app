...
import RestaurantOwnerEditor from "./RestaurantOwnerEditor";
import {
  ArrowUpRightIcon,
  CalendarIcon,
  CheckIcon,
...
const AMENITY_CAPTIONS: Partial<Record<string, string>> = {
  ...
  parqueadero: "Gratis",
  terraza: "Exterior",
  petFriendly: "Permitido",
  tarjeta: "Débito o crédito",
  comidaRapida: "10-20 min",
...
function priceRangeInfo(priceRange?: string): { symbol: string; label: string } | null {
  if (priceRange === "Bajo") return { symbol: "$", label: "Precio económico" };
  if (priceRange === "Medio") return { symbol: "$$", label: "Precio moderado" };
  if (priceRange === "Alto") return { symbol: "$$$", label: "Precio alto" };
  return null;
}
...
export default function RestaurantDetailView({
  ...
  const cuisineTags = (prestador.cuisineType ?? "").split(",").map((t) => t.trim()).filter(Boolean);
  ...
  const otherRestaurants = listPrestadoresByCategory(categoryKey)
      ...
      <div className="mx-auto max-w-8xl">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {canEdit ? (
            ...
            >
              {editMode ? "Salir del modo edición" : "Editar información del restaurante"}
            </button>
          ) : null}
        </div>
        ...
        {canEdit && editMode ? <RestaurantOwnerEditor prestadorId={prestador.id} categoryKey={categoryKey} onSaved={() => setDataVersion((v) => v + 1)} /> : null}
        ...
        <div className="mt-4 grid gap-6 lg:grid-cols-[1fr_360px]">
          <div>
            ...
            <div className="mt-6">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-brand-400">Restaurante</p>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-300">
                ...
                {averageRating !== null ? (
                  <span className="flex items-center gap-1.5">
                    <ClockIcon className="h-4 w-4 text-brand-400" /> {ratingLabel(averageRating)}
                  </span>
                ) : null}
                {priceInfo ? (
                  <span className="flex items-center gap-1.5">
                ...
                {prestador.status ? (
                  <span className="flex items-center gap-1.5">
                    <ClockIcon className="h-4 w-4 text-brand-400" /> {prestador.status === "Abierto" ? "Abierto ahora" : prestador.status}
                  </span>
                ) : null}
              </div>

              {cuisineTags.length > 0 ? (
                <p className="mt-2 text-sm text-slate-400">{cuisineTags.join(" · ")}</p>
              ) : null}
            ...
            {items.length > 0 ? (
              <div id="menu" className="mt-6 scroll-mt-20 rounded-2xl border border-forest-700 bg-forest-900 p-6">
                <h2 className="text-lg font-bold text-slate-100">Platos recomendados</h2>
                  ...
                  {visibleMenuItems.map((item) => (
                    <div key={item.id} className="overflow-hidden rounded-2xl border border-forest-700 bg-forest-950">
                      ...
                      <div className="p-3">
                        ...
                        {item.price ? <p className="mt-2 text-sm font-bold text-brand-400">{formatCOP(item.price)}</p> : null}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div id="opiniones" className="mt-6 scroll-mt-20 rounded-2xl border border-forest-700 bg-forest-900 p-6">
              ...
              {averageRating !== null ? (
                ...
                <p className="mt-4 text-sm text-slate-500">Aún no hay opiniones para este restaurante.</p>
              )}
                    ...
                    <div key={review.id} className="rounded-2xl border border-forest-700 bg-forest-950 p-4">
                      ...
                      {review.reply ? (
                        <div className="mt-3 rounded-xl bg-forest-900 p-3 text-xs text-slate-300">
                          <p className="font-semibold text-brand-400">Respuesta del restaurante</p>
          ...
          <div className="space-y-6 lg:sticky lg:top-6 lg:self-start">
            <div id="reserva-widget" className="scroll-mt-20 rounded-2xl border border-forest-700 bg-forest-900 p-6">
              <h2 className="text-lg font-bold text-slate-100">Reserva tu mesa</h2>
            ...
            {prestador.schedule || prestador.cuisineType || priceInfo || prestador.ambiance || prestador.dietaryOptions || prestador.paymentMethods ? (
              <div className="rounded-2xl border border-forest-700 bg-forest-900 p-6">
                <h2 className="text-center text-lg font-bold text-slate-100">Información importante</h2>
                <div className="mt-4 space-y-4">
                  {prestador.schedule ? (
                    <div className="flex items-start gap-3">
                      <ClockIcon className="mt-0.5 h-6 w-6 shrink-0 text-brand-400" />
                      <span>
                        <span className="block text-sm text-slate-400">Horario de atención</span>
                        <span className="text-base font-semibold text-slate-100">{prestador.schedule}</span>
                      </span>
                    </div>
                  ) : null}
                  {prestador.cuisineType ? (
                    <div className="flex items-start gap-3">
                      <ForkIcon className="mt-0.5 h-6 w-6 shrink-0 text-brand-400" />
                      <span>
                        <span className="block text-sm text-slate-400">Tipo de cocina</span>
                        <span className="text-base font-semibold text-slate-100">{prestador.cuisineType}</span>
                      </span>
                    </div>
                  ) : null}
                  {priceInfo ? (
                    <div className="flex items-start gap-3">
                      <TagIcon className="mt-0.5 h-6 w-6 shrink-0 text-brand-400" />
                      <span>
                        <span className="block text-sm text-slate-400">Rango de precios</span>
                        <span className="text-base font-semibold text-slate-100">{priceInfo.symbol} - {priceInfo.label}</span>
                      </span>
                    </div>
                  ) : null}
                  {prestador.ambiance ? (
                    <div className="flex items-start gap-3">
                      ...
                      <span>
                        <span className="block text-sm text-slate-400">Ambiente</span>
                        <span className="text-base font-semibold text-slate-100">{prestador.ambiance}</span>
                      </span>
                    </div>
                  ) : null}
                  {prestador.dietaryOptions ? (
                    <div className="flex items-start gap-3">
                      <CheckIcon className="mt-0.5 h-6 w-6 shrink-0 text-brand-400" />
                      <span>
                        <span className="block text-sm text-slate-400">Opciones</span>
                        <span className="text-base font-semibold text-slate-100">{prestador.dietaryOptions}</span>
                      </span>
                    </div>
                  ) : null}
            ...
            <div className="overflow-hidden rounded-2xl border border-forest-700 bg-forest-900 p-2">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/restaurantes%20.jpeg" alt="TripYopal IA" className="h-40 w-full object-contain" />
            </div>
          </div>
        </div>
      </div>
