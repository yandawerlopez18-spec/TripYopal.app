...
export default function BarOwnerEditor({ prestadorId, categoryKey, onSaved }: { prestadorId: string; categoryKey: string; onSaved: () => void }) {
  ...
  return (
    <div className="mt-6 rounded-3xl border border-brand-500/40 bg-forest-900 p-6">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-bold text-slate-100">Panel del administrador del bar</h2>
      </div>
      ...
      <div className={sectionClass}>
        ...
        <form onSubmit={handleGeneralSubmit} className="mt-4 grid gap-3 sm:grid-cols-2">
          <input className={inputClass} placeholder="Nombre del bar" value={generalForm.name} onChange={(e) => setGeneralForm({ ...generalForm, name: e.target.value })} />
          ...
          <input className={inputClass} placeholder="Horario (ej. 5:00 p.m. - 3:00 a.m.)" value={generalForm.schedule} onChange={(e) => setGeneralForm({ ...generalForm, schedule: e.target.value })} />
      ...
      <div className={sectionClass}>
        ...
        <p className="mt-1 text-sm text-slate-400">Agrega los eventos que tendrás próximamente en tu bar (noches temáticas, DJ invitados, etc.).</p>
        ...
        <form onSubmit={handleEventSubmit} className="mt-4 grid gap-3 border-t border-forest-700 pt-4 sm:grid-cols-2">
          ...
          <input className={inputClass} placeholder="Hora (ej. 9:00 p.m.)" value={eventForm.time} onChange={(e) => setEventForm({ ...eventForm, time: e.target.value })} />
          <ImageUploadField value={eventForm.imageUrl} onChange={(value) => setEventForm({ ...eventForm, imageUrl: value })} />
          <textarea
            className={`${inputClass} sm:col-span-2`}
            placeholder="Descripción breve (ej. DJ en vivo)"
